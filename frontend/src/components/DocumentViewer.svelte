...
<div class="flex flex-col bg-surface-bright min-h-screen text-on-secondary-fixed w-full max-w-[1200px] mx-auto border-x border-surface-container-high">

  <!-- Верхняя панель (Header) -->
  <header class="bg-white border-b border-surface-container-high h-14 w-full sticky top-0 z-30 flex items-center justify-between px-4">
    <button
      onclick={onBack}
      class="text-primary-container hover:bg-slate-100 rounded-full p-2 flex items-center justify-center transition-colors"
      aria-label="Назад к списку документов"
    >
      <span class="material-symbols-outlined">arrow_back</span>
    </button>
    <span class="font-sans font-bold text-sm md:text-base text-primary-container text-center flex-1 truncate px-4">
  ...
  <main class="flex-1 overflow-y-auto px-4 md:px-8 py-6 bg-white">
    ...
    <div class="mb-6 pb-6 border-b border-surface-container-high">
      <div class="flex flex-wrap items-center gap-2 mb-3">
        <span class="bg-primary text-white px-2.5 py-0.5 rounded-[0.25rem] font-mono text-[10px] font-bold">
          {getStatusRu(selectedDocument.status)}
        </span>
        <span class="text-on-surface-variant text-xs font-sans">
          Шифр: <strong class="text-slate-600 font-mono">{selectedDocument.documentNumber || 'Н/Д'}</strong>
        </span>
        <span class="text-on-surface-variant text-xs font-sans">•</span>
        <span class="text-on-surface-variant text-xs font-sans">
          Версия: <strong class="text-slate-600 font-mono">{selectedDocument.version || '1.0'}</strong>
        </span>
      </div>

      <h1 class="font-sans font-bold text-xl md:text-2xl text-primary-container leading-tight mb-4">
        {selectedDocument.title}
      </h1>

      <div class="grid grid-cols-1 md:grid-cols-2 gap-2 text-xs text-on-secondary-container bg-surface-bright p-3 rounded-[0.25rem] border border-surface-container-high">
    ...
    <!-- Аннотация / Описание -->
    <div class="mb-6 p-4 bg-surface-container-low border-l-4 border-primary rounded-r-[0.25rem]">
      <h3 class="text-xs font-bold text-primary-container uppercase tracking-wider mb-1 font-sans">Краткое описание</h3>
    ...
    <div class="space-y-4 text-sm text-slate-700 leading-relaxed font-sans border-b border-surface-container-high pb-8 mb-6">
      <h2 class="text-base font-bold text-primary-container pt-2 mb-2">1. Общие положения регламента</h2>
      <p>
      ...
      <h2 class="text-base font-bold text-primary-container pt-4 mb-2">2. Ключевые требования и стандарты</h2>
      <p>
    ...
    {#if showCommentsList}
      <section class="mt-6 mb-12">
        <div class="flex items-center justify-between border-b border-surface-container-high pb-2 mb-4">
          <h3 class="text-base font-bold text-primary-container font-sans flex items-center gap-2">
        ...
        {#if loadingComments}
          <div class="text-center py-4 text-on-surface-variant text-xs">Загрузка комментариев...</div>
        {:else if comments.length === 0}
          <div class="text-center py-6 bg-surface-container-low border border-dashed border-surface-container-high rounded-[0.25rem] text-on-surface-variant text-xs">
            Комментариев пока нет. Напишите первый отзыв!
          </div>
        {:else}
          <div class="space-y-3">
            {#each comments as c}
              <div class="bg-surface-bright border border-surface-container-high rounded-[0.25rem] p-3.5 flex flex-col gap-1 shadow-sm">
                <div class="flex items-center justify-between">
                  <span class="text-xs font-bold text-primary-container font-sans">
  ...
  <div class="bg-white/95 backdrop-blur border-t border-surface-container-high p-4 flex flex-col sm:flex-row justify-between items-center gap-3 sticky bottom-0 z-20">
    <button
      onclick={() => showCommentsList = !showCommentsList}
      class="w-full sm:w-auto flex items-center justify-center gap-2 px-4 py-2 border border-surface-container-high hover:bg-surface-container-low rounded-[0.25rem] text-primary-container text-xs font-bold font-sans transition-colors"
    >
    ...
    <div class="flex w-full sm:w-auto gap-2">
      ...
      {#if canEdit}
        <button
          ...
          class="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-4 py-2 bg-primary-container hover:bg-slate-800 text-white rounded-[0.25rem] text-xs font-bold font-sans transition-colors shadow-sm"
        >
      ...
      <button
        ...
        class="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-4 py-2 border border-primary hover:bg-blue-50 text-primary rounded-[0.25rem] text-xs font-bold font-sans transition-colors"
      >
      ...
      <button
        ...
        class="flex-1 sm:flex-initial flex items-center justify-center gap-1.5 px-4 py-2 bg-primary hover:bg-blue-600 text-white rounded-[0.25rem] text-xs font-bold font-sans transition-colors shadow-sm"
      >
  ...
  {#if showCommentModal}
    <div class="fixed inset-0 bg-on-secondary-fixed/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-lg border border-surface-container-high shadow-xl w-full max-w-md overflow-hidden flex flex-col">
        <div class="p-4 border-b border-surface-container-high bg-surface-bright flex justify-between items-center">
          <h3 class="font-sans font-bold text-sm text-primary-container">Добавить комментарий</h3>
          <button onclick={() => showCommentModal = false} class="text-on-surface-variant hover:text-slate-600" aria-label="Закрыть">
            <span class="material-symbols-outlined">close</span>
          </button>
        </div>
        <div class="p-4 space-y-3">
          <label for="comment-text" class="text-xs font-bold text-on-secondary-container block">ВАШ ОТЗЫВ ИЛИ ВОПРОС</label>
          <textarea
            id="comment-text"
            bind:value={commentText}
            rows="4"
            placeholder="Напишите ваш комментарий здесь..."
            class="w-full border border-slate-300 rounded-[0.25rem] p-2.5 text-sm focus:outline-none focus:border-primary focus:ring-1 focus:ring-primary font-sans"
          ></textarea>
          {#if commentError}
            <p class="text-red-500 text-xs">{commentError}</p>
          {/if}
        </div>
        <div class="p-4 border-t border-surface-container-high bg-surface-bright flex justify-end gap-2">
          <button
          ...
          <button
            ...
            class="px-4 py-2 bg-primary hover:bg-blue-600 text-white rounded-[0.25rem] text-xs font-bold transition-colors shadow-sm"
          >
            Опубликовать
          </button>
        </div>
  ...
  {#if showRequestModal}
    <div class="fixed inset-0 bg-on-secondary-fixed/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div class="bg-white rounded-lg border border-surface-container-high shadow-xl w-full max-w-md overflow-hidden flex flex-col">
        <div class="p-4 border-b border-surface-container-high bg-surface-bright flex justify-between items-center">
          <h3 class="font-sans font-bold text-sm text-primary-container">Запросить актуализацию</h3>
          <button onclick={() => showRequestModal = false} class="text-on-surface-variant hover:text-slate-600" aria-label="Закрыть">
            <span class="material-symbols-outlined">close</span>
          </button>
        </div>
        <div class="p-4 space-y-3">
          <label for="request-reason" class="text-xs font-bold text-on-secondary-container block">ОБОСНОВАНИЕ НЕОБХОДИМОСТИ ОБНОВЛЕНИЯ</label>
          <textarea
            id="request-reason"
  ...
  {#if showToast}
    <div class="fixed bottom-20 left-1/2 transform -translate-x-1/2 z-50 bg-primary-container text-white px-5 py-3 rounded-md shadow-lg border border-primary flex items-center gap-2 font-sans text-xs animate-bounce">
